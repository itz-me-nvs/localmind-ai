...
export default function OllamaChat({ chatList }: { chatList: ChatModel[] }) {
  ...
  return (
    <div ref={chatContainerRef} className="w-full flex space-y-2 p-4 overflow-y-auto">
        ...
        {chatList.map((item) => (
          ...
          >
            <div className={`flex w-full items-start gap-3 text-wrap break-words whitespace-pre-wrap ${
              ...
              <div
                className={`rounded-3xl ${item.role == "user" ? "px-5 py-2.5" : ''} text-wrap ${
                  item.role === "user"
              ...
              >
               {
                item.isError ? (
                    <div className="flex items-center justify-between border-red-300 dark:border-red-900 bg-background-message border p-3 rounded-lg">
                        ...
                        <p className="text-red-500">{item.name} + {' If this issue persists please contact us through our help center at https://ollama.ai/help'}</p>
                    </div>
